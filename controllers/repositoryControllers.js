...
async function addRepositoryInTracker(req, res) {
  ...
  try {
    ...
    if (error.message.includes('SELECT_FAILED')) {
      console.error(
        'DB insertError of addRepositoriesInTrackerFunction controller:',
        error
      );
      return res.status(500).json({
        error:
          '서버 처리 중 알 수 없는 오류가 발생했습니다. 다시 시도해주세요.',
      });
    } else if (error.message.includes('INSERT_FAILED')) {
      return res
        .status(404)
        .json({ error: '요청하신 GitHub 저장소를 찾을 수 없습니다.' });
    }
  }
}
...
async function deleteRepositoryInTracker(req, res) {
  ...
  try {
    const isdeleted = await deleteRepositoryToUserTrackedList(
      userId,
      githubRepoId
    );
